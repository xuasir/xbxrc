...
interface ApplyStreamVideoDisplayInput {
...
export function applyStreamVideoDisplay(input: ApplyStreamVideoDisplayInput): void {
...
export function bindRuntimeVideoFrameTracking(input: {
  playerElementId: string
  ...
  cleanups.push(
    input.runtime.events().on('stats.videoFrameProcessed', () => {
      input.onFrame()
    })
  )
