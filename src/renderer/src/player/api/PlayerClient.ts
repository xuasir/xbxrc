import { PlayerEvents, TypedEventEmitter } from './events'
import { DEFAULT_PLAYER_OPTIONS, PlayerClientOptions } from '../domain/config'
import { AudioRuntimeConfig, RendererRuntimeConfig, StreamStats } from '../domain/media'
import { ConnectParams, IceCandidateLike, SessionState, TransportRuntimeConfig } from '../domain/session'
import { InputFrame, InputRuntimeConfig } from '../domain/input'
import { KeyboardDriver, MouseKeyboardConfig } from '../infra/input/KeyboardDriver'
import { GamepadDriver } from '../infra/input/GamepadDriver'
import { NativeBridge } from '../infra/bridge/NativeBridge'
import { InputDriverLike, InputService } from '../app/input/InputService'
...
export interface PlayerClientInit extends Omit<Partial<PlayerClientOptions>, 'input' | 'audio' | 'renderer' | 'transport'> {
...
export interface PlayerInputController {
  ...
  pressButton(button: keyof InputFrame, durationMs: number): void;
...
export class PlayerClient {
    ...
    private readonly nativeBridge = new NativeBridge()
    ...
    constructor(options: PlayerClientInit) {
        ...
        this.options = {
            ...
            mouseKeyboardConfig: options.mouseKeyboardConfig ?? defaults.mouseKeyboardConfig,
        }
        this.keyboardDriver = new KeyboardDriver(this.options.mouseKeyboardConfig)
        this.gamepadDriver = this.createInputDriver(options.inputDriver)
        ...
        if (options.inputDriver && typeof options.inputDriver.setApplication === 'function') {
            options.inputDriver.setApplication(this)
        }
        ...
        this.sessionService = new SessionService(this.options, this.inputService, this.mediaService, this.emitter)
    ...
    async createOffer(): Promise<RTCSessionDescriptionInit> {
        const offer = await this.sessionService.createOffer()
        return offer
    }
    ...
    getMouseKeyboardConfig(): MouseKeyboardConfig {
        return this.options.mouseKeyboardConfig
    }
    ...
    input(): PlayerInputController {
        return {
            ...
            setKeyboardInputEnabled: (enabled) => this.setKeyboardInputEnabled(enabled),
    ...
    private createInputDriver(inputDriver?: any): InputDriverLike {
        if (inputDriver === null) {
            return {
                start: () => undefined,
                stop: () => undefined,
        ...
        if (inputDriver !== undefined) {
            return inputDriver as InputDriverLike
        }
        return new GamepadDriver(this.keyboardDriver, {
            onGamepadAdded: (index) => this.sessionService.getControlChannel()?.sendGamepadAdded(index),
            onGamepadRemoved: (index) => this.sessionService.getControlChannel()?.sendGamepadRemoved(index),
