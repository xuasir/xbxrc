...
use crate::{
    spawn_gilrs_input_runtime_with_source, GilrsBackendConfig, GilrsDeviceDescriptor,
    GilrsInputEvent, GilrsInputEventKind, GilrsSource, GilrsSourceInitError,
    OhMyGamepadDesktopKeyboardListenerConfig, RealGilrsSource,
};
...
pub struct OhMyGamepadServiceConfig {
    ...
    pub backend: GilrsBackendConfig,
...
impl Default for OhMyGamepadServiceConfig {
    fn default() -> Self {
        Self {
            ...
            backend: GilrsBackendConfig::default(),
...
impl OhMyGamepadService {
    pub fn spawn(config: OhMyGamepadServiceConfig) -> Result<Self, GilrsSourceInitError> {
        let (physical_source, rumble_handle) = RealGilrsSource::new()?;
    ...
    pub fn apply_command(
        ...
        // 统一收口服务级命令，便于后续宿主层或测试层复用同一协议。
        match command {
            OhMyGamepadServiceCommandDto::RefreshSnapshot => {}
            OhMyGamepadServiceCommandDto::SetRouteTarget { target } => {
                self.set_route_target(target)?;
            }
    ...
    fn wait_for_device_state(
        ...
        let mut last_snapshot = self.snapshot_with_strategy_sync()?;
        for _ in 0..SERVICE_SYNC_RETRY_COUNT {
            if last_snapshot.pads.iter().any(|pad| {
    ...
    fn reconcile_sampling_binding(
        ...
        let current = snapshot.bindings.first().cloned().unwrap_or_default();
        if current == desired {
            return Ok(false);
        }
...
fn normalize_core_config(
    ...
    if core.bindings.is_empty() {
        // 服务层把多手柄采样策略折叠成 ohmygamepad-core 可执行的一条 pad 绑定。
...
fn simulated_descriptor(descriptor: &SimulatedGamepadDescriptor) -> GilrsDeviceDescriptor {
...
#[cfg(test)]
#[path = "oh_my_gamepad_service.test.rs"]
mod tests;
