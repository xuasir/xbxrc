...
use crate::{
    ...
    XbxEngineRecoveryPreset, XbxEngineRecoveryRuntimeConfig, XbxEngineRecoverySignals,
...
pub struct XbxEngineWebRtcRuntimeConfig {
    ...
    pub recovery_preset: XbxEngineRecoveryPreset,
...
impl XbxEngineWebRtcRuntimeConfig {
    pub fn from_mode(mode: XbxStreamingMode) -> Self {
        let (remb, video, recovery) = match mode {
            XbxStreamingMode::CloudGaming => (
                Some(50_000), // 50Mbps 满足 1080p60 Peak
                ...
                XbxEngineRecoveryPreset::CloudConservative,
            ),
            XbxStreamingMode::LocalHost => (
                Some(100_000), // 100Mbps 本地链路全开
                XbxEngineVideoPipelineRuntimeConfig {
                    nack_window_ms: 200,
                    ...
                    idle_timeout_ms: 100, // 超激进熔断
                },
                XbxEngineRecoveryPreset::LanLowLatency,
            ),
            XbxStreamingMode::CloudHost => (
                Some(30_000), // 典型 VPS 带宽对等，30M 足够。
                XbxEngineVideoPipelineRuntimeConfig {
                    nack_window_ms: 600,
                    nack_retry_interval_ms: 100,
                    nack_max_retry_count: 10,
                    jitter_buffer_min_delay_ms: 30,
                    jitter_buffer_max_delay_ms: 50,
                    jitter_buffer_max_packets: 2048,
                    idle_timeout_ms: 250, // 容忍跨区长 RTT
                },
                XbxEngineRecoveryPreset::CloudAggressive,
            ),
        };

        Self {
            mode,
            forced_remb_kbps: remb,
            adaptive_remb_enabled: false, // 既然是锁死模式，默认关闭动态探测
            ...
            recovery_preset: recovery,
            recovery: XbxEngineRecoveryRuntimeConfig::from_preset(recovery),
        }
    }
}
...
impl Default for XbxEngineWebRtcRuntimeConfig {
    fn default() -> Self {
        Self::from_mode(XbxStreamingMode::CloudGaming)
    }
}
...
fn map_streaming_mode(mode: xbxengine_protocol::XbxStreamingModeDto) -> XbxStreamingMode {
    match mode {
        xbxengine_protocol::XbxStreamingModeDto::CloudGaming => XbxStreamingMode::CloudGaming,
        xbxengine_protocol::XbxStreamingModeDto::LocalHost => XbxStreamingMode::LocalHost,
        xbxengine_protocol::XbxStreamingModeDto::CloudHost => XbxStreamingMode::CloudHost,
    }
}
