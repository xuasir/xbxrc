...
use super::display::{
    current_fresh_anchor_recovered_at_ms_from_stats, current_playback_recovered_at_ms_from_stats,
};
...
pub(crate) fn recovery_exit_path_from_stats(
    ...
    let playback_recovered_fresh = current_playback_recovered_at_ms_from_stats(stats)
        .is_some_and(|at_ms| (now_ms - at_ms).max(0.0) <= thresholds.degraded_decode_age_ms);
    let host_present_fresh = stats
        .latest_video_host_present_time_ms
        .is_some_and(|at_ms| (now_ms - at_ms).max(0.0) <= thresholds.degraded_decode_age_ms);
    let host_output_advancing =
        stats.host_frame_present_epoch > 0 && playback_recovered_fresh && host_present_fresh;
