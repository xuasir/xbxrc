...
pub(crate) struct RecoveryCoordinator {
    ...
    /// 上次检查的IDR解码时间（用于检测新的IDR解码事件）
    last_idr_decoded_at_ms: Option<f64>,
...
impl RecoveryCoordinator {
    ...
    pub(crate) fn propose_from_owner_signal(
        ...
        if Self::uses_connectivity_escalation(signal.reason) {
    ...
    fn update_inflight_status(&mut self, runtime_stats: &Mutex<XbxEngineMediaRuntimeStats>) {
        RuntimeStatsSink::read_shared(runtime_stats, |stats| {
            ...
            if self.coordinator.state_machine().is_idr_in_flight() {
                let should_clear = Self::check_idr_completed(stats, self.last_idr_decoded_at_ms);

                if should_clear {
                    self.coordinator.on_idr_decoded();
                    // 更新最后检查的时间
                    self.last_idr_decoded_at_ms = stats
    ...
    fn check_idr_completed(
        ...
        // 证据2：episode状态为decoded
        let episode_decoded = stats
            .latest_keyframe_request_episode
            .as_ref()
            .is_some_and(|ep| ep.status == "decoded");

        if episode_decoded {
            return true;
        }

        // 证据3：有新的IDR帧解码（检查first_keyframe_decoded_at_ms）
        let has_new_idr_decoded = stats
            .latest_keyframe_request_episode
            .as_ref()
            .and_then(|ep| ep.first_keyframe_decoded_at_ms)
            .is_some_and(|decoded_at| last_checked_at_ms.map_or(true, |last| decoded_at > last));

        if has_new_idr_decoded {
            return true;
        }
    ...
    fn uses_connectivity_escalation(reason: VideoEscalationReason) -> bool {
        matches!(
            reason,
...
mod tests {
    ...
    #[test]
    fn reconnect_budget_snapshots_reflect_before_and_after_state() {
        ...
        assert_eq!(proposal.budget_after.reconnect_budget_used, 1);
    }
