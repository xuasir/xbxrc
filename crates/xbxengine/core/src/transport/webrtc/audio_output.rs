...
impl XbxRemoteAudioPlaybackSession {
    pub(crate) fn start(
        ...
        let decode_task =
            spawn_audio_decode_task(track, runtime_stats, shared_state, stop_receiver);
...
impl AudioPlaybackSharedState {
    ...
    fn fill_output_f32(
        ...
        // 这里保持一个简单的最近邻重采样，先保证 Rust runtime 的可播性。
    ...
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                next_packet = track.read_rtp() => {
                    let Ok((rtp, _)) = next_packet else {
                        break;
                    };
                    ...
                    match decoder.decode_float(&rtp.payload, &mut decoded_pcm, false) {
                        ...
                        Err(error) => {
                            crate::xbx_log_warn!(
                                "[xbxengine][webrtc-rs][audio] opus decode failed: {error}"
                            );
                        }
                    }
                }
