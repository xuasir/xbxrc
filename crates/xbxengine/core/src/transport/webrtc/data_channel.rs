...
const STREAM_CONTROL_KEYFRAME_INTERVAL_MS: u64 = 2000;
const STREAM_CONTROL_GAMEPAD_ADDED_DELAY_MS: u64 = 500;

// 协议构建逻辑 (从已删除的 network_profile.rs 迁移)
...
fn is_handshake_ack_payload(payload: &str) -> bool {
    payload.contains("handshake-ack")
}

fn build_control_keyframe_request_payload() -> String {
    r#"{"type":"video_keyframe_request"}"#.to_string()
}

fn build_control_decoder_reset_payload() -> String {
    r#"{"type":"decoder_reset"}"#.to_string()
}
...
fn build_control_gamepad_changed_payload(added: bool) -> String {
    format!(r#"{{"type":"gamepad_changed","added":{}}}"#, added)
}

fn build_input_metadata_packet(_seq: u32, _time: f64) -> Vec<u8> {
    vec![0] // 占位实现
}

fn build_input_gamepad_packet(_seq: u32, _time: f64, _frames: &[LogicalPadSnapshotDto]) -> Vec<u8> {
    vec![0] // 占位实现
}
