...
- `ohmygamepad-host`
  - 提供 `GamepadRuntimeHost::shared()` 单实例入口
  - Tauri 与 xbxengine-core 都通过此入口获取 gamepad 实例
- `xbxengine`
...
以下 crate 已归档到 `archived/` 目录，不再作为主线使用：

- `xbxengine-app` - 原生窗口宿主（仅作参考保留）
- `xbxengine-api` - N-API 绑定层（已废弃）
- `ohmygamepad-bridge-napi` - N-API 绑定层（已废弃）
