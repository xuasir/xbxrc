...
impl DataService {
    pub fn new(
        ...
        Self {
            auth_provider,
            inner: Mutex::new(DataServiceInner {
...
impl DataProvider for DataService {
    async fn get_user_profile(&self) -> Result<DataUserProfile, String> {
        ...
        let Some(session) = session else {
            ...
            inner.profile_service.clear_cached_profile()?;
            return inner.profile_service.get_cached_profile(0);
        };
        ...
        let inner = self.inner.lock().await;
        inner.profile_service.get_cached_profile(session.app_level)
    }
    ...
    async fn get_hosts(&self) -> Result<Vec<DataHostSummary>, String> {
        ...
        let inner = self.inner.lock().await;
        let hosts = inner.host_service.get_hosts(&session, &web_api).await?;
    ...
    async fn get_xcloud_titles(&self) -> Result<Vec<DataXcloudTitleSummary>, String> {
        ...
        inner.xcloud_service.get_titles(&session).await
    }
}
...
impl DataService {
    ...
    async fn resolve_web_api_client(
        ...
        let mut inner = self.inner.lock().await;
        inner.web_api_provider.get_or_create(session)
    }
}
