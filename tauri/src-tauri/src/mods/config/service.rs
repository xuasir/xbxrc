...
impl ConfigService {
    ...
    fn normalize_number(key: &str, value: &Value, fallback: i64, min: i64, max: i64) -> Value {
        let parsed = if let Some(val) = value.as_i64() {
            ...
            log::warn!("Config key '{}' missing or invalid, using fallback: {}", key, fallback);
            fallback
        };
        Value::from(parsed)
    }
    ...
    fn normalize_string_enum(key: &str, value: &Value, fallback: &str, allowed: &[&str]) -> Value {
        ...
        log::warn!("Config key '{}' missing or invalid enum, using fallback: {}", key, fallback);
        Value::from(fallback.to_string())
    }

    fn normalize_display_options(key: &str, value: &Value, fallback: &Value) -> Value {
        let Some(map) = value.as_object() else {
            log::warn!("Config key '{}' missing or invalid display options, using fallback", key);
        ...
        let sharpness = if let Some(val) = map.get("sharpness").and_then(Value::as_i64) {
            ...
            log::warn!("Config key '{}.sharpness' missing or invalid, using fallback: {}", key, default_map["sharpness"].as_i64().unwrap_or(2));
            default_map["sharpness"].as_i64().unwrap_or(2)
        };
        let saturation = if let Some(val) = map.get("saturation").and_then(Value::as_i64) {
            val.clamp(0, 200)
        } else {
            log::warn!("Config key '{}.saturation' missing or invalid, using fallback: {}", key, default_map["saturation"].as_i64().unwrap_or(100));
            default_map["saturation"].as_i64().unwrap_or(100)
        };
        let contrast = if let Some(val) = map.get("contrast").and_then(Value::as_i64) {
            val.clamp(0, 200)
        } else {
            log::warn!("Config key '{}.contrast' missing or invalid, using fallback: {}", key, default_map["contrast"].as_i64().unwrap_or(100));
            default_map["contrast"].as_i64().unwrap_or(100)
        };
        let brightness = if let Some(val) = map.get("brightness").and_then(Value::as_i64) {
            val.clamp(0, 200)
        } else {
            log::warn!("Config key '{}.brightness' missing or invalid, using fallback: {}", key, default_map["brightness"].as_i64().unwrap_or(100));
        ...
        match key {
            ...
            | "server_credential" => {
                if let Some(val) = value.as_str() {
                    ...
                    log::warn!("Config key '{}' missing or invalid, using fallback: {}", key, fallback.as_str().unwrap_or(""));
            ...
            | "use_vulkan" => {
                if let Some(val) = value.as_bool() {
                    ...
                    log::warn!("Config key '{}' missing or invalid, using fallback: {}", key, fallback.as_bool().unwrap_or(false));
                    Value::from(fallback.as_bool().unwrap_or(false))
                }
            }
            "resolution" => Self::normalize_number(key, value, fallback.as_i64().unwrap_or(720), 720, 1081),
            "xhome_bitrate" | "xcloud_bitrate" | "audio_bitrate" => {
                Self::normalize_number(key, value, fallback.as_i64().unwrap_or(20), 0, 200)
            }
            "polling_rate" => Self::normalize_number(key, value, fallback.as_i64().unwrap_or(250), 1, 1000),
