...
2. Run `scripts/summarize_runtime_trace.py <trace.jsonl>` to get a compact overview.
3. Read `references/log-schema.md` when you need field semantics.
4. Read `references/analysis-playbook.md` when you need the project-specific workflow, output contract, or heuristics.
5. Re-open the raw trace around the key `seq` / `tsMs` window before making conclusions.
...
4. Reconstruct the primary timeline with concrete `seq`, `tsMs`, `domain`, and `event` anchors.
5. Identify the first abnormal signal, not only the final failure symptom.
6. Correlate front-end, Tauri, service, and `xbxengine` observations before inferring causality.
7. Distinguish evidence from hypothesis. State uncertainty explicitly when the trace is incomplete.
8. Recommend the next diagnostic step only after listing the missing evidence.

## Use The Bundled Resources

- Use `scripts/summarize_runtime_trace.py` first for row counts, domains, sessions, log levels, and suspicious rows.
- Read `references/log-schema.md` for the JSONL envelope, row categories, and interpretation rules.
- Read `references/analysis-playbook.md` for project-specific analysis steps, common focus areas, and reporting format.
